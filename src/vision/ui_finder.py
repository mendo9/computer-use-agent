...
class UIElement:
    ...
    yolo_detection: Optional[Detection] = None
    
    # Text info (if available)
    text_detection: Optional[TextDetection] = None
...
class UIFinder:
    ...
    def find_element_by_text(self, image: np.ndarray, text_query: str, search_radius: int = 100) -> List[UIElement]:
        """
    ...
    def find_clickable_elements(self, image: np.ndarray) -> List[UIElement]:
        ...
        clickable_classes = {
            "mouse", "remote", "cell phone", "laptop", "keyboard",
            "tv", "book", "clock"  # These might represent UI elements in screenshots
        }
        ...
        text_detections = self.ocr_reader.read_text(image)
        button_keywords = ["button", "click", "submit", "ok", "cancel", "yes", "no", "save", "delete"]
    ...
    def _combine_nearby_elements(self, ui_elements: List[UIElement], max_distance: int = 50) -> List[UIElement]:
    ...
    def _find_nearby_visual_elements(self, center_point: Tuple[int, int], 
                                   detections: List[Detection], max_distance: int) -> List[Detection]:
    ...
    def get_element_screenshot(self, image: np.ndarray, element: UIElement, padding: int = 5) -> np.ndarray:

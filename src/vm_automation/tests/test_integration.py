...
from ..poc_main import VMAutomationPOC
from ..agents import POCTarget


@pytest.mark.integration
@pytest.mark.mock
@pytest.mark.asyncio
async def test_full_poc_workflow_mock(mock_poc_target, golden_set_data, phoenix_tracer, deepeval_evaluator, test_output_dir):
    ...
    poc = VMAutomationPOC(mock_poc_target, use_mock=True)
    ...
    assert session_summary["screenshots_count"] >= golden_set_data["minimum_screenshots"], "Not enough screenshots captured"
    assert session_summary["errors_count"] <= expected_final_state["errors_count_max"], "Too many errors occurred"
...
async def test_poc_error_handling(mock_poc_target, phoenix_tracer):
    ...
    poc = VMAutomationPOC(invalid_target, use_mock=True)  # Still use mock for testing
...
async def test_full_poc_with_real_vm(poc_target, golden_set_data, phoenix_tracer, deepeval_evaluator, test_output_dir):
    ...
    poc = VMAutomationPOC(poc_target, use_mock=False)
...
async def test_poc_performance_benchmark(mock_poc_target, golden_set_data):
    ...
    assert avg_time <= golden_set_data["maximum_execution_time"], f"Average execution time too high: {avg_time:.2f}s"
    assert max_time <= golden_set_data["maximum_execution_time"] * 1.5, f"Max execution time too high: {max_time:.2f}s"

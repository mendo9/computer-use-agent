...
class InputActions:
    ...
    def drag(self, start_x: int, start_y: int, end_x: int, end_y: int) -> ActionResult:
        ...
        try:
            ...
            return ActionResult(True, f"Dragged from ({start_x}, {start_y}) to ({end_x}, {end_y})", time.time())
    ...
    def scroll(self, x: int, y: int, direction: str = "up", clicks: int = 3) -> ActionResult:
        ...
        try:
            ...
            time.sleep(self.action_delay)
            return ActionResult(True, f"Scrolled {direction} {clicks} times at ({x}, {y})", time.time())

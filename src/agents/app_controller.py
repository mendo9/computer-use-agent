...
class AppControllerTools:
    ...
    def __init__(self, session: VMSession, vm_target: VMTarget, shared_components: Dict[str, Any]):
        ...
        self.session.log_action("App Controller initialized with shared components from VM Navigator")
    ...
    def find_target_element_with_retry(self, element_description: str, max_retries: int = 3) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_retries):
            try:
                self.session.log_action(f"Looking for element: {element_description} (attempt {attempt + 1})")
                ...
                self.session.add_screenshot(screenshot, f"Searching for {element_description} (attempt {attempt + 1})")
                ...
                if elements:
                    ...
                    self.session.log_action(f"Found element '{element_description}' at {best_element.center}")
                ...
                for element in clickable_elements:
                    if element.text and element_description.lower() in element.text.lower():
                        self.session.log_action(f"Found clickable element with matching text: '{element.text}'")
                        return {
                            "success": True,
                            "element": {
                                "text": element.text,
                ...
                for element in self.ui_finder.find_ui_elements(screenshot):
                    if element.text:
                        ...
                        if any(keyword in element_text_lower for keyword in keywords):
                            self.session.log_action(f"Found element by keyword match: '{element.text}'")
                            return {
                                "success": True,
                                "element": {
                                    "text": element.text,
                ...
                if attempt < max_retries - 1:
                    self.session.log_action(f"Element '{element_description}' not found, retrying...")
                    time.sleep(2.0)
                ...
                if attempt < max_retries - 1:
                    ...
                    error_msg = f"Element search error: {str(e)}"
                    self.session.add_error(error_msg)
                    return {"success": False, "error": error_msg}
        
        return {"success": False, "error": f"Element '{element_description}' not found after {max_retries} attempts"}
    ...
    def verify_action_outcome(self, expected_outcomes: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Action outcome verification error: {str(e)}"
            self.session.add_error(error_msg)
            return {"success": False, "error": error_msg}
                ...
                if scroll_attempt > 0:
                    ...
                    if screenshot:
                        ...
                        if not scroll_result.success:
                            self.session.log_action(f"Scroll attempt {scroll_attempt} failed: {scroll_result.message}")
                            continue
                        
                        time.sleep(1.0)  # Wait for scroll to complete
                
                # Try to find element
                find_result = self.find_target_element_with_retry(element_description, max_retries=1)
                
                if find_result["success"]:
                    self.session.log_action(f"Found element after {scroll_attempt} scrolls")
                    return find_result
            
            return {"success": False, "error": f"Element not found after {max_scrolls} scroll attempts"}
            
        except Exception as e:
            error_msg = f"Scroll and search error: {str(e)}"
            self.session.add_error(error_msg)
            return {"success": False, "error": error_msg}
...
class AppControllerAgent:
    ...
    async def execute_button_click_workflow(self, expected_outcomes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for step_name, step_func in steps:
                ...
                if step_name == "Find target element":
                    ...
                    result = self.tools.find_target_element_with_retry(self.vm_target.target_button_text)
                    ...
                    if expected_outcomes:
                        ...
                        result = {"success": True, "message": "Action outcome verification skipped (no expected outcomes specified)"}
    ...
    async def execute_form_filling_workflow(self, form_fields: List[Dict[str, str]], submit_button: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        try:
            self.session.log_action(f"Starting form filling workflow with {len(form_fields)} fields")

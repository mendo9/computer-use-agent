...
class VMNavigatorTools:
    ...
    def capture_screen_with_retry(self, description: str = "Screen capture", max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    self.session.log_action(f"Screenshot error (attempt {attempt + 1}): {e}, retrying...")
                    time.sleep(1.0)
                else:
                    error_msg = f"Screen capture error after {max_retries} attempts: {str(e)}"
                    self.session.add_error(error_msg)
                    return {"success": False, "error": error_msg}
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                result = self.verifier.verify_page_loaded(
                    screenshot, 
                    self.vm_target.expected_desktop_elements,
                    timeout=5
                )
    ...
    def find_application_with_retry(self, app_name: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                self.session.log_action(f"Looking for application: {app_name} (attempt {attempt + 1})")
                ...
                self.session.add_screenshot(screenshot, f"Searching for {app_name} (attempt {attempt + 1})")
                ...
                if attempt < max_retries - 1:
                    self.session.log_action(f"Application search error (attempt {attempt + 1}): {e}")
                    time.sleep(2.0)
                else:
        ...
        return {"success": False, "error": f"Application {app_name} not found after {max_retries} attempts"}
    ...
    def launch_application_verified(self, element_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if verification.success:
                ...
                return {"success": False, "error": f"Could not verify application launch: {verification.message}"}
        ...
        try:
            ...
            for field_name, expected_value in expected_patient_info.items():
                ...
                if expected_upper in banner_text_combined:
                    ...
                    self.session.add_error(f"✗ Patient {field_name} NOT FOUND: expected '{expected_value}'")
            ...
            if len(verified_fields) >= min_required_matches:
                self.session.log_action(f"SAFETY CHECK PASSED: {len(verified_fields)} patient identifiers verified")
                return {
                    "success": True,
            ...
            error_msg = f"Patient verification error: {str(e)}"
            self.session.add_error(error_msg)
            return {
                "success": False, 
                "error": error_msg,
    ...
    def verify_application_loaded_enhanced(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.vm_target.expected_app_elements:
                result = self.verifier.verify_page_loaded(
                    screenshot, 
                    self.vm_target.expected_app_elements
                )
            ...
            verification_results.append(("ui_elements", ui_elements_found, f"Found {len(elements)} UI elements"))
            
            # 3. Check window title or specific text
            app_name_elements = self.ui_finder.find_element_by_text(screenshot, self.vm_target.target_app_name.replace(".exe", ""))
            app_title_found = len(app_name_elements) > 0
            verification_results.append(("app_title", app_title_found, f"App title found: {app_title_found}"))
            ...
            if successful_checks >= (total_checks // 2 + 1):  # Majority must pass
                self.session.log_action(f"Application verification passed: {successful_checks}/{total_checks} checks")
                return {
                    "success": True, 
...
class VMNavigatorAgent:
    ...
    def __init__(self, session: VMSession, vm_target: VMTarget, shared_components: Optional[Dict[str, Any]] = None):
        """
    ...
    async def execute_navigation(self, patient_info: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            steps = [
                ...
                ("Capture initial screen", lambda: self.tools.capture_screen_with_retry("Initial desktop")),
                ("Wait for desktop", self.tools.wait_for_desktop_loaded),
                ("Find application", lambda: self.tools.find_application_with_retry(self.vm_target.target_app_name)),
            ...
            for step_name, step_func in steps:
                ...
                if step_name == "Launch application":
                    ...
                    if patient_info:
                        ...
                        result = {"success": True, "message": "Patient verification skipped (no patient info provided)"}

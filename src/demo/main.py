...
def setup_models():
    ...
    if not yolo_path.exists():
        ...
        print("  uv run python -m ocr.setup_models")
        return False
...
def create_vnc_session(host: str, port: int = 5900, password: str | None = None) -> Session:
    ...
    conn_adapter = ConnectionAdapter(
        vm_connection=vnc_connection, host=host, port=port, credentials=credentials
    )
    ...
    # Create vision components using superior VM implementations
    models_dir = Path(__file__).parent.parent / "ocr" / "models"
    yolo_path = models_dir / "yolov8s.onnx"
    det = YOLODetector(model_path=str(yolo_path), confidence_threshold=0.5)
    ocr = OCRReader(use_gpu=False, lang="en")
...
def create_rdp_session(
    ...
    conn_adapter = ConnectionAdapter(
        vm_connection=rdp_connection, host=host, port=port, credentials=credentials
    )

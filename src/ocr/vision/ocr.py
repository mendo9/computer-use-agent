...
class OCRReader:
    ...
    def __init__(self, use_gpu: bool = False, lang: str = "en"):
        ...
        self.ocr = paddleocr.PaddleOCR(lang=lang)
        print(f"PaddleOCR initialized (GPU: {use_gpu}, Lang: {lang})")

    def read_text(
        self, image: np.ndarray, region: tuple[int, int, int, int] | None = None
    ) -> list[TextDetection]:
        """
        ...
        # Run OCR
        try:
            results = self.ocr.ocr(cropped_image)
    ...
    def find_text(
        self, image: np.ndarray, target_text: str, threshold: float = 0.8
    ) -> list[TextDetection]:
        """
    ...
    def draw_text_detections(
        self, image: np.ndarray, text_detections: list[TextDetection]
    ) -> np.ndarray:
    ...
    def get_text_near_point(
        self, image: np.ndarray, point: tuple[int, int], radius: int = 50
    ) -> list[TextDetection]:
        """

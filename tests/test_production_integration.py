...
from src.production import VMAutomationProduction, ProductionConfig
from src.agents.shared_context import VMTarget, VMSession
from .mock_components import create_mock_components, setup_patient_safety_test_scenario, setup_error_scenario
    ...
    def _setup_mock_agents(self):
        ...
        class MockVMNavigatorTools:
            ...
            def find_application_with_retry(self, app_name, max_retries=3):
                ...
                if elements:
                    return {"success": True, "element": {"center": elements[0].center, "bbox": elements[0].bbox}}
        ...
        class MockVMNavigator:
            ...
            async def execute_navigation(self, patient_info=None):
                ...
                if patient_info:
                    ...
                    self.session.log_action(f"✓ Patient name verified: {patient_info.get('name', '')}")
                    self.session.log_action(f"✓ Patient MRN verified: {patient_info.get('mrn', '')}")
        ...
        self.vm_navigator = MockVMNavigator(self.session, self.config.to_vm_target(), shared_components)
        self.app_controller = MockAppController(self.session, self.config.to_vm_target(), shared_components)
...
async def test_session_reporting_and_phi_handling(mock_automation):
    ...
    if mock_automation.config.log_phi:
        ...
        if mock_automation.config.patient_name in action_log_text:
            ...
            assert "[PATIENT_NAME]" in action_log_text or "patient information configured" in action_log_text.lower()

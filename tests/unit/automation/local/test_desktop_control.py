...
class TestDesktopControl:
    ...
    def test_capture_screen_success(self, mock_imread, mock_subprocess):
        ...
        mock_subprocess.assert_called_once_with(
            ["screencapture", "-x", str(desktop.screenshot_path)], check=True, capture_output=True
        )
    ...
    def test_capture_window_interactive(self, mock_imread, mock_subprocess):
        ...
        assert np.array_equal(image, mock_image)
        mock_subprocess.assert_called_once_with(
            ["screencapture", "-w", "-x", str(desktop.screenshot_path)], check=True
        )
    ...
    def test_click_right_button(self, mock_subprocess):
        ...
        expected_script = (
            'tell application "System Events" to tell (click at {50, 75}) to key down control'
        )
        mock_subprocess.assert_called_once_with(
            ["osascript", "-e", expected_script], check=True, capture_output=True, text=True
        )
    ...
    def test_click_with_cliclick_available(self, mock_subprocess, mock_which):
        ...
        mock_subprocess.assert_called_once_with(
            ["cliclick", "c:100,200"], check=True, capture_output=True, text=True
        )
    ...
    def test_double_click_success(self, mock_sleep, mock_click):
        ...
        mock_click.assert_has_calls([call(150, 250, "left"), call(150, 250, "left")])
    ...
    def test_type_text_with_quotes(self, mock_subprocess):
        ...
        expected_script = (
            r'tell application "System Events" to keystroke "Hello \"World\" and \'test\'"'
        )
        mock_subprocess.assert_called_once_with(
            ["osascript", "-e", expected_script], check=True, capture_output=True, text=True
        )

...
class MockScreenCapture(ScreenCapture):
    ...
    def capture_screen(self) -> np.ndarray | None:
        ...
        if self.capture_count == 0:
            ...
            cv2.putText(
                mock_desktop, "App", (185, 205), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1
            )
            ...
            cv2.putText(
                mock_desktop,
                "Patient: John Doe | MRN: 123456 | DOB: 01/01/1980",
                (60, 115),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
            ...
            cv2.putText(
                mock_desktop, "Name Field", (105, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1
            )
...
class MockUIFinder:
    ...
    def find_ui_elements(self, screenshot) -> list[UIElement]:
        ...
        return [
            UIElement(
                "detected", (50, 50, 700, 600), (375, 325), 0.9, description="Mock app window"
            ),
            UIElement(
                "text",
                (400, 300, 500, 350),
    ...
    def find_clickable_elements(self, screenshot) -> list[UIElement]:
        ...
        return [
            UIElement(
                ...
                0.9,
                text="Submit",
                description="Mock clickable button",
            )
        ]
...
class MockOCRReader:
    ...
    def read_text(self, image: np.ndarray, region: tuple[int, int, int, int] | None = None):
        """Mock text reading"""
...
class MockVerifier:
    ...
    def verify_page_loaded(self, screenshot, indicators, timeout=5) -> VerificationResult:
        ...
        found_indicators = [
            ind for ind in indicators if ind in ["Desktop", "Start", "Submit", "MockDesktop"]
        ]
...
def setup_patient_safety_test_scenario(
    mock_components: dict[str, Any], patient_name: str = "John Doe", mrn: str = "123456"
):
    """Setup mock components for patient safety testing"""
    # Add patient info to UI finder
    mock_components["ui_finder"].add_mock_element(
        patient_name,
        UIElement(
            element_type="text",
            bbox=(60, 90, 200, 130),
